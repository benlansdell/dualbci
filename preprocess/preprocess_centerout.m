function processed = preprocess_centerout(nevfile, labviewfile, binsize, threshold, offset, fn_out, verbose, units)
	...
	tspks = spikemuas;
	nB = size(binnedspikes,1);

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	%Import trial info from mat file%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	for idx = 1:length(data.trial)
		endbin = floor(data.trial(idx).tout/binsize);
		startbin = floor(data.trial(idx).tcenter/binsize);
		target(startbin:endbin) = data.trial(idx).targ;
	end
