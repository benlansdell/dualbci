...
for i = 1:length(conds)
	...
	for j = 1:length(nevs)
		...
		if (exist(nevfile, 'file') == 2) & (exist(matfile, 'file') == 2)
			%Preprocess data
			processed = preprocess_spline_target(nevfile, matfile, binsize, threshold, offset);		
			...
			data = filters_sprc_pos_target_lv(processed_mua, nK_sp, nK_pos, nK_tar, dt_sp, dt_pos);
			%Fit the GLM
