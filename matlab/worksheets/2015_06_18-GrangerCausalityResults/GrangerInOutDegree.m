...
'INNER JOIN `experiment_tuning` al ON bcgc.`nev file` = al.`1DBCrecording` '...
'INNER JOIN fits f ON f.`nev file` = al.manualrecording '...
'INNER JOIN estimates_granger ge ON ge.fromunit = bcgc.`fromunit` AND f.fitID = ge.fitID '...
'WHERE f.modelID = 2 '...
...
'INNER JOIN fits f '...
'ON  f.id = ge.id '...
'WHERE f.unit NOT IN (SELECT bc.unit FROM bci_units bc WHERE bc.ID = f.`nev file`) '...
'AND f.`nev file` IN (SELECT ID FROM bci_units) '...
'AND f.`modelID` = 2'])
