...
%Things to search and replace for future analyses based on this file:
%-the date to save plots in
%-sprc_relpos => sprc_[model]
%-sprcrelpos => sprc[model]

%Make sure to copy across all results files from computer that ran the expt
...
mat_sprc_relpos = load('./expts/2dmanualpos_sprc_relpos_lv_def.mat');
mat_sprc = load('./expts/2dmanualpos_sprc_def.mat');
expts_sprc_relpos = mat_sprc_relpos.expts;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Note their change in deviance between sprc and sprc_relpos_lv%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%For each unit, find all experiments containing that unit
for i = 1:nE
	expt_sprc = expts_sprc(i);
	expt_sprc_relpos = expts_sprc_relpos(i);
	unitsstart = cellfun(@str2num, expt_sprc_relpos.modelstart.unitnames);
	unitsend = cellfun(@str2num, expt_sprc_relpos.modelend.unitnames);
	...
	for j = 1:nEl
		...
		if any(unitsstart == j) & any(unitsend == j)
			...
			DP(j, i, 3) = expt_sprc_relpos.modelstart.dev{startidx};
			DP(j, i, 4) = expt_sprc_relpos.modelend.dev{endidx};
...
for idx = 1:nEl
	...
	if sum(nz) > 0
		...
		x = (nzExpts(:,2)-nzExpts(:,4))./nzExpts(:,2);
		y = (nzExpts(:,1)-nzExpts(:,3))./nzExpts(:,1);
		...
		saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_unit_' num2str(idx) '_n_' num2str(n) '.eps'], 'eps', [5 20])
	end
end
...
ylabel('Day 0')
title(['Time driving BCI'])		
saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_all_N_' num2str(N) '.eps'], 'eps', [5 20])
...
for idx = 1:nEl
	...
	if sum(nz) > 0
		...
		ylabel('DP')
		title(['Time driving BCI'])		
		saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_date_unit_' num2str(idx) '_n_' num2str(n) '.eps'], 'eps', [5 20])
	end
end
...
ylabel('DP')
title(['Time driving BCI'])		
saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_all_date_N_' num2str(N) '.eps'], 'eps', [5 20])
...
for idx = 1:nEl
	...
	if sum(nz) > 0
		...
		ylabel('DP')
		title(['Time driving BCI'])		
		saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_abs_date_unit_' num2str(idx) '_n_' num2str(n) '.eps'], 'eps', [5 20])
	end
end
...
ylabel('DP')
title(['Time driving BCI'])		
saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprcrelpos_abs_all_date_N_' num2str(N) '.eps'], 'eps', [5 20])
