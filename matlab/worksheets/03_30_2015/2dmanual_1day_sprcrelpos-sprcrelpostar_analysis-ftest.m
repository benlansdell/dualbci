...
%-sprc_posvel => sprc_[model]
%-sprcposvel => sprc[model]
...
mat_sprc_posvel = load('./expts/2dmanualpos_sprc_relpos_target_lv_def.mat');
mat_sprc = load('./expts/2dmanualpos_sprc_relpos_lv_def.mat');
expts_sprc_posvel = mat_sprc_posvel.expts;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Note their change in deviance between sprc and sprc_posvel_lv%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%For each unit, find all experiments containing that unit
for i = 1:nE
	expt_sprc = expts_sprc(i);
	expt_sprc_posvel = expts_sprc_posvel(i);
	unitsstart = cellfun(@str2num, expt_sprc_posvel.modelstart.unitnames);
	unitsend = cellfun(@str2num, expt_sprc_posvel.modelend.unitnames);
	...
	for j = 1:nEl
		...
		if any(unitsstart == j) & any(unitsend == j)
			...
			DP(j, i, 3) = expt_sprc_posvel.modelstart.dev{startidx};
			DP(j, i, 4) = expt_sprc_posvel.modelend.dev{endidx};
...
for idx = 1:nEl
	...
	if sum(nz) > 0
		...
		saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprcrelpos-sprcrelpostar_unit_' num2str(idx) '_n_' num2str(n) '.eps'], 'eps', [5 20])
	end
end
...
ylabel('Day 0')
title(['Time driving BCI'])		
saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprc1_all_N_' num2str(N) '.eps'], 'eps', [5 20])
...
for idx = 1:nEl
	...
	if sum(nz) > 0
		...
		ylabel('DP')
		title(['Time driving BCI'])		
		saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprcrelpos-sprcrelpostar_date_unit_' num2str(idx) '_n_' num2str(n) '.eps'], 'eps', [5 20])
	end
end
...
ylabel('DP')
title(['Time driving BCI'])		
saveplot(gcf, ['./worksheets/03_30_2015/plots/2dmanualpos_1day_sprc-sprc1_all_date_N_' num2str(N) '.eps'], 'eps', [5 20])
