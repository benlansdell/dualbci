...
%%%%%%%%%%%%%%%%
runIdx = 16;
nevfile = './testdata/20130123SpankyUtah004.nev';
fn_out = './worksheets/04_10_2015/gpfa_bci_2013-01-23_extended';
labviewfile = './testdata/Spanky_2013-01-23-1320.mat';
binsize = 0.001;
offset = 0.0;
threshold = 5;
...
%Get dwell time
load(labviewfile);
dwelltime = data.params.dwellTime(1)/binsize;

%Time bins which occur within a trial
spikes_bci = processed_bci.binnedspikes;
trials_bci = [1; diff(sum(abs(processed_bci.target),2)>0)~=-1];
...
plotEachDimVsTime_colhoriz(seqTrain_bci, 'xorth', result_bci.binWidth, quads_bci, 'nPlotMax', 100, 'dwelltime', dwelltime);
...
runIdx = 17;
fn_out = './worksheets/04_10_2015/gpfa_manual_2013-01-23_extended';
nevfile = './testdata/20130123SpankyUtah002.nev';
labviewfile = './testdata/Spanky_2013-01-23-1320.mat';
binsize = 0.001;
offset = 0.0;
threshold = 5;
...
trials_man = [1, diff(sum(abs(processed_man.target),2)>0)~=-1];
...
% Plot each dimension of neural trajectories versus time
plotEachDimVsTime_colhoriz(seqTrain_man, 'xorth', result_man.binWidth, quads_man, 'nPlotMax', 100, 'dwelltime', dwelltime);
saveplot(gcf, [fn_out '_EachDimVsTime_colhoriz.eps'], 'eps', [10 6])
