...
fn_out = './worksheets/04_21_2015/gpfa_centerout_2014-01-16_tstart-tend';
labviewfile = './testdata/20140116_ExperimentalParamCorrected.mat';

binsize = 0.001;
offset = 0.0;
threshold = 5;
processed_bci = preprocess_centerout_tstarttend(nevfile, labviewfile, binsize, threshold, offset);

%Time bins which occur within a trial
spikes_bci = processed_bci.binnedspikes;
trials_bci = processed_bci.target>0;
intrial = [];
for idx = 1:length(processed_bci.trialdata)
	intrial(idx,1) = floor(1000*(processed_bci.trialdata(idx).tcenter-processed_bci.trialdata(idx).tstart)/20);
	intrial(idx,2) = floor(1000*(processed_bci.trialdata(idx).tout-processed_bci.trialdata(idx).tstart)/20);
end
...
plot2D_coloct_intrial(seqTrain_bci, 'xorth', targ, intrial, 'dimsToPlot', 1:3, 'nPlotMax', 10);
plot2svg([fn_out '_plot2D_coloct_intrial.svg'])
plot3D_coloct_intrial(seqTrain_bci, 'xorth', targ, intrial, 'dimsToPlot', 1:3, 'nPlotMax', 10);
plot2svg([fn_out '_plot3D_coloct_intrial.svg'])
plotEachDimVsTime_coloct_intrial(seqTrain_bci, 'xorth', result_bci.binWidth, targ, intrial, 'nPlotMax', 10);
plot2svg([fn_out '_EachDimVsTime_coltarg_intrial.svg'])

%saveplot(gcf, [fn_out '_plot3D_coloct.eps'])
plot2D_coloct_mean(seqTrain_bci, 'xorth', targ, 'dimsToPlot', 1:3, 'nPlotMax', 100000);
...
fn_out = './worksheets/04_21_2015/gpfa_centerout_2014-02-03_tstart-tend';
...
plot2svg([fn_out '_plot3D_coloct_intrial.svg'])
plotEachDimVsTime_coloct_intrial(seqTrain_bci, 'xorth', result_bci.binWidth, targ, intrial, 'nPlotMax', 10);
plot2svg(gcf, [fn_out '_EachDimVsTime_coltarg_intrial.eps'])

plotEachDimVsTime_colmeantarget(seqTrain_bci, 'xorth', result_bci.binWidth, targ, 'nPlotMax', 10000);
plot2svg([fn_out '_EachDimVsTime_colmeantarget.svg'])
plot3D_coloct_mean(seqTrain_bci, 'xorth', targ, 'dimsToPlot', 1:3, 'nPlotMax', 10);
plot2svg([fn_out '_plot3D_coloct_mean.svg'])
