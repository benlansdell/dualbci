%Script to run 1D linear tuning model, tracking units between recordings. 
%Model 38 fits linear model to only the manual direction, not to the BC direction in dual control
modelID = 38;
blackrock = './blackrock/';
labviewpath = './labview/';
...
for idx = 1:108
	...
	if ~strcmp(MCnevfile1, 'null') & ~strcmp(MCnevfile2, 'null') & ~strcmp(BCnevfile1, 'null') & ~strcmp(DCnevfile, 'null')
		...
		processLinearVel1D_paired(conn, modelID, blackrock, labviewpath, MCnevfile1, BCnevfile1, MCnevfile2, DCnevfile, expt_id, paramcode);
	else
