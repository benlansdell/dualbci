...
eval(paramcode);
%Generate a MC distribution of torque output...
%Probably 20 files is enough to come up with a good enough distribution of 
%torque in MC
torque = [];
torque_vel = [];
...
%	display(['Reading ' nevfile])
%	processed = preprocess_spline(nevpath, binsize, threshold, offset);
%	torque = [torque(:,:); processed.torque];
%	torque_vel = [torque_vel(:,:); processed.dtorque];
%end
%Save torque output
reftorqdist = 'reftorquedist.mat';
%save(['./worksheets/2015_11_03-bcireportplots/' reftorqdist], 'torque', 'torque_vel');
load(['./worksheets/2015_11_03-bcireportplots/' reftorqdist]);
...
for idx = 1:nR
	...
	if exist([blackrock nevfile], 'file') & exist([blackrock BCnevfile], 'file') & exist([blackrock DCnevfile], 'file') & exist([blackrock MC2nevfile], 'file')
		processLinearControlVelocity(conn, modelID, blackrock, nevfile, paramcode, threshold, reftorqdist, allunits, rerun);
		processLinearControlVelocity(conn, modelID, blackrock, BCnevfile, paramcode, threshold, reftorqdist, allunits, rerun);
		processLinearControlVelocity(conn, modelID, blackrock, DCnevfile, paramcode, threshold, reftorqdist, allunits, rerun);
		processLinearControlVelocity(conn, modelID, blackrock, MC2nevfile, paramcode, threshold, reftorqdist, allunits, rerun);
	else
