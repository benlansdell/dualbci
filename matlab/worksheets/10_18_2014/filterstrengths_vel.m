...
fn_out = './worksheets/10_18_2014/data_vel.mat';
N = 200000;
binsize = 0.002;
dur = N*binsize;
dt_sp = binsize;
...
k_RU = -100.0*exp(-6*t_pos);
k_FE = 50.0*exp(-5*t_pos);
...
for idx = 1:nAlpha
	...
	processed{idx} = generate_glm_data_torque_vel(pre, k_const_guess, k_sp, k_RU_sc, k_FE_sc, dt_sp, dt_pos, N, binsize);
	...
	processed{idx} = generate_glm_data_torque_vel(pre, k_const(idx), k_sp, k_RU_sc, k_FE_sc, dt_sp, dt_pos, N, binsize);
...
for i = 1:nAlpha
	...
	for j = 1:length(nK_poss);
		...
		data = filters_sp_vel(processed{i}, nK_sp, nK_pos, dt_sp, dt_pos);
		...
		fn_out2 = ['./worksheets/10_18_2014/plots/filterstrengths_vel_alpha_' num2str(alphas(i)) '_dtpos_' num2str(dt_pos)];
...
colorbar
saveplot(gcf, './worksheets/10_18_2014/plots/filterstrengths_vel_dev.eps')
