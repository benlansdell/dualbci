...
%Variance of BC units in manual control recording 
var_BC1 = [];
var_BC2 = [];
...
for idx = 1:nR
	...
	%Get the variance for the two BC units 
	u = bciunits{1};
	mse_var = fetch(exec(conn, ['SELECT f1.`mse out` FROM '...
	'`fits` f1 WHERE f1.unit = ' u ' AND '...
	'f1.`nev file` = "' mcnev '" AND f1.modelID = 19']));
	var_BC1(idx) = mse_var.Data{1};
	u = bciunits{1};
	mse_var = fetch(exec(conn, ['SELECT f1.`mse out` FROM '...
	'`fits` f1 WHERE f1.unit = ' u ' AND '...
	'f1.`nev file` = "' mcnev '" AND f1.modelID = 19']));
	var_BC2(idx) = mse_var.Data{1};
...
%Plot BC units variance vs pa between MC and BCI space
scatter(max(var_BC1, var_BC1), paMCBCI)
xlabel('Variance of BC units in MC')
ylabel('principal angle bw MC and control space')
saveplot(gcf, './worksheets/2016_02_07-GPFApaired/analyseGPFA_varMC_paMCBCI.eps')
