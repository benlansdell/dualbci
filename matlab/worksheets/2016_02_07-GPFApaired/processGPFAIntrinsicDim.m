function [analysis_id, ll, D] = processGPFAIntrinsicDim(conn, modelID, blackrock, labviewpath, labviewfile, MCnevfile1, BCnevfile1, DCnevfile, expt_id, paramcode)
	...
	if ~strcmp(previous.Data{1}, 'No Data')
		...
		%llBC = runGPFAIntPaired(conn, analysis_id, 2, modelID, blackrock, labviewpath, labviewfile, BCnevfile1, allunits, expt_id, paramcode);
		%llDC = runGPFAIntPaired(conn, analysis_id, 3, modelID, blackrock, labviewpath, labviewfile, DCnevfile, allunits, expt_id, paramcode);
		%ll = {llMC, llBC, llDC};
		ll = {llMC};
	end
end
