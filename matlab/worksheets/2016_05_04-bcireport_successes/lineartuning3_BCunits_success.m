...
'ON flin5.id = fl5.id '...
'WHERE flin1.modelID = 41 AND flin2.modelID = 41 AND flin3.modelID = 41 AND flin5.modelID = 41 ' ...
...
saveplot(gcf, './worksheets/2016_05_04-bcireport_successes/tuningstrengthR2R2outBCunits.eps', 'eps', [10 6])
...
saveplot(gcf, './worksheets/2016_05_04-bcireport_successes/tuningstrengthVR2R2outBCunits.eps', 'eps', [10 6])


%Tuning angles (velocity)
all_data = fetch(exec(conn, ['SELECT fl1.dir, fl2.dir, fl3.dir, fl5.dir, et1.`tuning_type`, rec1.`successrate`, rec2.`successrate`, bci1.angle, bci2.angle FROM '...
'`experiment_tuning` et1 '...
'INNER JOIN `fits` flin1 '...
'ON flin1.`nev file` = et1.`manualrecording`'...
'INNER JOIN `fits_linear` fl1 '...
...
'INNER JOIN `bci_units` bci1 '...
'ON bci1.`ID` = et1.`1DBCrecording` AND bci1.`unit` = flin1.unit '...
'INNER JOIN `bci_units` bci2 '...
'ON bci2.`ID` = et1.`dualrecording` AND bci2.`unit` = flin1.unit '...
'WHERE flin1.modelID = 40 AND flin2.modelID = 40 AND flin3.modelID = 40 AND flin5.modelID = 40 ' ...
'AND flin1.unit = flin2.unit AND flin2.unit = flin3.unit AND flin2.unit = flin5.unit ' ...
'AND EXISTS (SELECT * FROM `bci_units` bci WHERE bci.`ID` = et1.`1DBCrecording` AND bci.unit = flin1.unit) '...
...
ylabel('\theta DC')
title(['corr rotated: ' num2str(corrsrotated(5)) ' corr unrotated: ' num2str(corrsunrotated(5))])
saveplot(gcf, './worksheets/2016_05_04-bcireport_successes/tuningangleVR2R2outBCunits.eps', 'eps', [10 6])
...
ylabel('\theta DC')
title(['corr rotated: ' num2str(corrsrotated(5)) ' corr unrotated: ' num2str(corrsunrotated(5))])
saveplot(gcf, './worksheets/2016_05_04-bcireport_successes/tuninganglePR2R2outBCunits.eps', 'eps', [10 6])
