...
for idx = 1:nAlpha
	...
	for j = 1:length(nK_poss)
		...
		processed{idx,j} = generate_glm_data_torque(pre, k_const(idx), k_sp, k_RU_sc, k_FE_sc, dt_sp, dt_pos, N, binsize);
...
for i = 1:nAlpha
	...
	for j = 1:length(nK_poss);
		...
		fn_out2 = ['./worksheets/10_22_2014/plots/filterstrengths_alpha_' num2str(alphas(i)) '_dtpos_' num2str(dt_pos)];
	...
	subplot(3,1,3)
	xlabel('time (s)')
	fn_out3 = ['./worksheets/10_22_2014/plots/filterstrengths_alpha_' num2str(alphas(i)) '.eps'];
	...
	xlabel('time(s)')
	fn_out3 = ['./worksheets/10_22_2014/plots/filterstrengths_alpha_' num2str(alphas(i)) '_fourier.eps'];
	saveplot(gcf, fn_out3, 'eps', [9 6]);
end
...
colorbar
saveplot(gcf, './worksheets/10_22_2014/plots/filterstrengths_dev.eps')
