function data = filters_sp_vel(processed, nK_sp, nK_pos)
	%Prepare spike and torque data for GLM which includes spike history and cursor velocity (x_1, x_2) filters:
	%
	%	y(i) ~ Pn(g(eta_i))
	%
	%where
	...
	%	nK_sp = 50; 
	%	nK_pos = 10;
	%	data = filters_sp_vel(pre.processed, nK_sp, nK_pos);
	...
	for idx=1:nU 
		...
		for j = (nK_sp+1):(nB-nK_pos)
			...
			torqueRU = processed.dtorque(j:(j+nK_pos-1),1);
			torqueFE = processed.dtorque(j:(j+nK_pos-1),2);
			%Form stim vector
