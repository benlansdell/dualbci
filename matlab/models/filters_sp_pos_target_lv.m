function data = filters_sp_pos_target_lv(processed, nK_sp, nK_pos, nK_tar, dt_sp, dt_pos, dt_tar)
	...
	%
	%Usage:
	%	data = filters_sp_pos_target_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_tar = (optional, default = binze in processed structure) step size of target position filter
	%		in seconds. Must be a multiple of the data's binsize
	%   
	%Output:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sp_pos_target_lv(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
	...
	data.k{3,3} = dt_pos;
	data.k{4,1} = 'RU tar'; 
	data.k{4,2} = (1:nK_tar) + nK_sp + 2*nK_pos;
	data.k{4,3} = dt_tar;
	data.k{5,1} = 'FE tar'; 
	data.k{5,2} = (1:nK_tar) + nK_sp + 2*nK_pos + nK_tar;
	data.k{5,3} = dt_tar;
	%Record specifically which indices are spike history indices for model simulation
	data.sp_hist = data.k{1,2};

	startbin = max((nK_sp*steps_sp+1),(nK_tar*steps_tar+1));
	...
	for idx=1:nU 
		...
		for j = startbin:endbin
			...
			targetRU = processed.target((j-(nK_tar-1)*steps_tar:steps_tar:j),1);
			targetFE = processed.target((j-(nK_tar-1)*steps_tar:steps_tar:j),2);			
			%Form stim vector
			data.X(idx,j,:) = [shist' torqueRU' torqueFE' targetRU' targetFE'];
		end
	end
