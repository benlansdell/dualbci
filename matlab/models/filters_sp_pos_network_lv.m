function data = filters_sp_pos_network_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	...
	%
	%Usage:
	%	data = filters_sp_pos_network_lv(processed, unit, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 1/60;
	%	dt_pos = 1/60;
	%	data = filters_sp_pos_network_lv(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
	...
	data.k{nU+1,2} = (1:nK_pos) + nU*nK_sp;
	data.k{nU+1,3} = dt_pos;
	data.k{nU+2,1} = 'FE pos'; 
	data.k{nU+2,2} = (1:nK_pos) + nU*nK_sp + nK_pos;
