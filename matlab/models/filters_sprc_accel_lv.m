function data = filters_sprc_accel_lv(processed, nK_sp, nK_acc, dt_sp, dt_acc)
	...
	%
	%Usage:
	%	data = filters_sprc_accel_lv(processed, nK_sp, nK_acc, dt_sp, dt_acc)
	%     
	%Input:
	%	processed = structure output from one of the preprocess functions.
	%	nK_sp = number of timebins used for spike history filter
	%	nK_acc = number of timebins used for cursor trajectory filters (on in x and y axis)
	...
	%	dt_acc = (optional, default = binsize in processed structure) step size of position filter in
	...
	%			and last nK_sp and nK_acc timebins respectively.
	...
	%	dt_sp = 0.002;
	%	dt_acc = 0.05;
	%	data = filters_sprc_accel_lv(pre.processed, nK_sp, nK_acc, dt_sp, dt_acc);

	if (nargin < 4) dt_sp = processed.binsize; end
	if (nargin < 5) dt_acc = processed.binsize; end

	%Check dt's specified are valid
	assert(rem(dt_sp,processed.binsize)==0, 'Invalid dt_sp. Must be a multiple of binsize');
	assert(rem(dt_acc,processed.binsize)==0, 'Invalid dt_acc. Must be a multiple of binsize');
	steps_sp = dt_sp/processed.binsize;
	steps_acc = dt_acc/processed.binsize;
	...
	data.k{1,3} = dt_sp;
	data.k{2,1} = 'RU pos'; 
	data.k{2,2} = (1:nK_acc) + nK_rc;
	data.k{2,3} = dt_acc;
	data.k{3,1} = 'FE pos'; 
	data.k{3,2} = (1:nK_acc) + nK_rc + nK_acc;
	data.k{3,3} = dt_acc;
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_acc*steps_acc)
			%(past) spike history
			shist = project_rc(processed.binnedspikes(j-nK_sp*steps_sp:steps_sp:j-steps_sp, idx), rcbasis);
			%(future) torque trajectory
			torqueRU = processed.ddcursor(j:steps_acc:(j+nK_acc*steps_acc-1),1);
			torqueFE = processed.ddcursor(j:steps_acc:(j+nK_acc*steps_acc-1),2);
	...
	data.X = data.X(:,(nK_sp*steps_sp+1):(nB-nK_acc*steps_acc),:); %(nkt+1:end-nkt,:);
	data.y = processed.binnedspikes((nK_sp*steps_sp+1):(nB-nK_acc*steps_acc), :)';
