function [scoreFE, scoreRU] = correlation_intrial_nev(nevfile, matfile, fn_out, threshold, binsize, sigma_fr, sigma_trq)
	%correlation_intrial_nev	Function to fit the following model to spike and torque data:
	...
	%		will be fit.
  %   Only compute correlations for data within a trial
	%
	%		Usage:
	%			[scoreFE, scoreRU] = correlation_intrial_nev(nevfile, matfile, fn_out, threshold, binsize, sigma_fr, sigma_trq)
	%
	% 		Input:
	%			nevfile = file to process
  %     matfile = labview file containing trial information
  ...
  %     matfile = './testdata/Spanky_2013-01-17-1325.mat';
	...
	%			sigma_fr = 0; 
	%			sigma_trq = 0.25;
	%			fn_out = './worksheets/tuning/intrial/20130117SpankyUtah001';
	%			correlation_intrial_nev(nevfile, matfile, fn_out, threshold, binsize, sigma_fr, sigma_trq);
	
	%Optional arguments
	if (nargin < 4)	threshold = 1; end
	if (nargin < 5) binsize = 0.002; end
	if (nargin < 6) sigma_fr = 0; end
