function processed = preprocess_spline_target(nevfile, labviewfile, binsize, threshold, offset, fn_out, verbose, units)
	...
	%		labviewfile = mat file containing trial information corresponding to nev file
	...
	%		target = [nB x 2] coordinates of target position (zero between trials [when there's no target])
	%		cursor = [nB x 2] coordinates of cursor position (should match torque)
	%
	%Test code:
	...
	%		offset = 0.0;
	%		threshold = 5;
	%		fn_out = './worksheets/diagnostics/plots/test_spline_pre_target';
	%		verbose = 0;
	%		processed = preprocess_spline_target(nevfile, labviewfile, binsize, threshold, offset, fn_out, verbose);
	...
	for trial = trials
		...
		if strcmp(nevfiles, nevfiletrial)
			...
			L = size(trial.cursor,1);
			cursor_trial(startbin:endbin,:) = resample(trial.cursor, endbin-startbin+1, L);
			%Might have to use trial.lag value here, too, but will plot to check
			nevoffset = trial.offset;
		end
	end
	...
	%If plotting diagnostics, then plot to check cursor and torque and cursor trial are aligned
	if isstr(fn_out)
		figure
		tt = 1:10000;
		subplot(3,1,1)
		plot(tt*binsize,-torque(tt,1),tt*binsize, cursor(tt,1),tt*binsize, cursor_trial(tt,1))
		title('RU')
		subplot(3,1,2)
		plot(tt*binsize,-torque(tt,2),tt*binsize, cursor(tt,2),tt*binsize, cursor_trial(tt,2))
		title('FE')
		legend('Torque', 'Cursor', 'Cursor: trial')
		xlabel('time (s)')
		subplot(3,1,3)
		maxlag = 4/binsize;
		torquecursor = xcorr(-torque(:,1), cursor(:,1), maxlag);
		tt = ((1:length(torquecursor))-length(torquecursor)/2)*binsize;
		plot(tt,torquecursor)
		maxcorr = find(torquecursor==max(torquecursor));
		title(['Max at t=' num2str(tt(maxcorr))]);
		xlabel('time(s)')
		saveplot(gcf, [fn_out '_torque_cursor.eps'], 'eps', [10 6]);
	end
