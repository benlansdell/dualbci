function processLinearCursorVel(conn, modelID, blackrock, labview, nevfile, matfile, paramcode, threshold, units, rerun)
	...
	d = ['processLinearCursorVel: Fitting model ' num2str(modelID) ' nevfile ' nevfile];
	display(d);
	writelog(logfile, d);
	...
	for idx = 1:nU
		...
		if ~strcmp(previous.Data{1}, 'No Data')
			if ~rerun 
				d = ['processLinearCursorVel: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Skipping'];
				display(d);
				writelog(logfile, d);
				continue
			elseif length(previous.Data) > 1
				d = ['processLinearCursorVel: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Trying to overwrite '...
					' old fit, but found too many matching fits for this model to unambiguously replace with new data. Skipping'];
				display(d);
				writelog(logfile, d);
				continue				
			else
				d = ['processLinearCursorVel: rerun = 1: Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Deleting and '...
				're-entering.'];
				display(d);
				writelog(logfile, d);
