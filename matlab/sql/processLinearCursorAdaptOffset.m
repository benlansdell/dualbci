function processLinear(conn, modelID, blackrock, labview, nevfile, matfile, paramcode, units)
	...
	%Preprocess data
	if nargin < 8
		processed = preprocess_spline_lv_adaptoffset(nevpath, matpath, binsize, threshold);
	else
		processed = preprocess_spline_lv_adaptoffset(nevpath, matpath, binsize, threshold, 0, 0, units);
	end
