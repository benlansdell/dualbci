function processLinear(conn, modelID, blackrock, nevfile, paramcode, units)
	nevpath = [blackrock nevfile];
	%Load parameters
	eval(paramcode);
	%Preprocess data
	if nargin < 6
		processed = preprocess_smooth_adaptoffset(nevpath, binsize, sigma_fr, sigma_trq, threshold);
	else
		processed = preprocess_smooth_adaptoffset(nevpath, binsize, sigma_fr, sigma_trq, threshold, 0, 0, units);
	end
	...
	recdur = size(processed.rates,1)*processed.binsize;
	if recdur < (dur + testdur)
		display(['Recording too short. ' num2str(recdur) ' is less than the requested ' num2str(dur+testdur) '. Skipping'])
		return;
	end
