function processMVGCmanualPairedNoBCI(conn, modelID, blackrock, labviewpath, nevfile1, BCnevfile, nevfile2, expt_id, paramcode)
	...
	units = fetch(exec(conn, ['SELECT u.`unit` FROM `units` u WHERE `nev file` = "'...
		nevfile1 '" AND u.`firingrate` > ' num2str(threshold)...
' AND NOT EXISTS (SELECT * FROM bci_units bci WHERE bci.ID = `nev file` AND bci.unit = u.unit)']));
	units = units.Data;
