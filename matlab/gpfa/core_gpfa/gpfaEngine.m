...
%        John Cunningham  jcunnin@stanford.edu
  ...
  if ~isempty(seqTest) % check if there are any test trials
    if estParams.notes.RforceDiagonal
      [seqTest, DD] = cosmoother_gpfa_viaOrth_fast(seqTest, estParams, 1:xDim);
    else
      [seqTest, DD] = cosmoother_gpfa_viaOrth(seqTest, estParams, 1:xDim);
    end
  end
  ...
  vars  = who;
  fprintf('Saving %s...\n', fname);
  save(fname, vars{~ismember(vars, {'yAll'})}, '-v7.3');
