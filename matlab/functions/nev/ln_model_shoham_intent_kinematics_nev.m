function [r2 LNr2 loglikelihoods, lp] = ln_model_shoham_intent_kinematics_nev(nevfile, matfile, fn_out, kernellength, polydeg)
	%ln_model_shoham_intent_kinematics_nev	Function to fit the following model to spike and torque data:
	...
	%
	%		Performs additional step of only including data within a trial AND
	%		transforms torque data so that trial target position is at the origin, and the trial starting position is
	%		directly below it. (A simple model of intentionality)
	%		
	%		Usage:
	%			[r2 LNr2 loglikelihoods lp] = ln_model_shoham_intent_kinematics_nev(nevfile, fn_out, kernlelength, polydeg)
	%
	% 		Input:
	...
	%			kernellength = 4;
	%			polydeg = 4;
	%			fn_out = './worksheets/intent_kinematics/20130117SpankyUtah001';
	%			[r2 LNr2 ll lp] = ln_model_shoham_intent_kinematics_nev(nevfile, matfile, fn_out, kernellength, polydeg);
    ...
    for i=1:2
		for j=1:kl
	    	...
	    	laggedrtorque = [laggedrtorque, rtorque(j:end-kl+j,i)];
    	end
    end
    ...
    kernelrRU = zeros(nU, kl);
    kernelrFE = zeros(nU, kl);
    r2 = zeros(nU,1);
    LNr2 = zeros(nU,1);
	for j=1:nU
		%Estimate parameters as a linear model
		X = [laggedtorque(:,1:kl), laggedtorque(:,(kl+1):(2*kl)), laggedrtorque(:,1:kl), laggedrtorque(:,(kl+1):(2*kl))];
		Y = rates(:,j);
		...
		kernelFE(j,1:kl) = coeffs(1*kl+1:2*kl);
		kernelrRU(j,1:kl) = coeffs(2*kl+1:3*kl);
		kernelrFE(j,1:kl) = coeffs(3*kl+1:end);
	...
	for i=1:nU
    	...
    	subplot(3,4,9);
		hold on
		plot((0:kl-1)*binsize, squeeze(kernelrRU(i,1:kl)));
		xlabel('time (s)')
		ylabel('k_{rot} [RU]')
		subplot(3,4,10)
		hold on
		plot((0:kl-1)*binsize, squeeze(kernelrFE(i,1:kl)));
		xlabel('time (s)')
		ylabel('k_{rot} [FE]')
		...
		ylabel('spikes/s');
		legend('Actual', 'Model', 'Location', 'NorthOutside');
		saveplot(gcf, [fn_out '_LNmodel_' unitnames{i} '_klength_' num2str(kl) '.eps'], 'eps', [12 8]);
	end
...
%		plot(bins/nbins*(umax-umin)+umin, meanspikes);
	...
	nparams = polydeg+4*kernellength;
	ndatapts = szRates;
