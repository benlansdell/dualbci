function [maxR2 sumR2] = regression_intrial_nev(nevfile, matfile, fn_out, kernellength, binsize, sigma_fr, sigma_trq, offset, verbosity)
	%regression_intrial_nev	Function to fit the following model to spike and torque data:
	...
	%		Only fit model based on data occurring within a trial
	%
	%		Usage:
	%			[maxR2, sumR2] = regression_intrial_nev(nevfile, matfile, fn_out, kernellength, binsize, sigma_fr, sigma_trq, offset, verbosity)
	%
	% 		Input:
	%			nevfile = file to process
	%			matfile = labview data file containing trial information
	...
	%			offset = 0;
	%			verbosity = 1;
	%			fn_out = './worksheets/tuning/intrial_regress/20130117SpankyUtah001';
	%			regression_intrial_nev(nevfile, matfile, fn_out, kernellength, binsize, sigma_fr, sigma_trq, offset, verbosity);
	
	%Optional arguments
	if (nargin < 4)	kernellength = 6; end
	if (nargin < 5) binsize = 0.05; end
	if (nargin < 6) sigma_fr = 0.25; end
	if (nargin < 7) sigma_trq = 0.25; end
	if (nargin < 8) offset = 0; end
	if (nargin < 9) verbosity = 0; end
