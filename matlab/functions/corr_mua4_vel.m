function corr4 = corr_mua4(nevfiles, binsize, fn_out, sigma, offset)
	...
	%			bs = 0.1;
	%			fn = './worksheets/diagnostics/plots/test_corr_mua4_vel_20130117SpankyUtah001';
	%			corr4 = corr_mua4_vel(nevfiles, bs, fn);
	...
	for idx = 1:nE
		...
		pindices2 = vels(:,2)>0;
		nindices2 = vels(:,2)<0;
		%[corr4(idx, 1), p4(idx,1)] = corr(subplus(vels(:,1)), rates(:,idx));
		%[corr4(idx, 2), p4(idx,2)] = corr(-subplus(-vels(:,1)), rates(:,idx));
		%[corr4(idx, 3), p4(idx,3)] = corr(subplus(vels(:,2)), rates(:,idx));
		%[corr4(idx, 4), p4(idx,4)] = corr(-subplus(-vels(:,2)), rates(:,idx));
		[corr4(idx, 1), p4(idx,1)] = corr(vels(pindices1,1), rates(pindices1,idx));
		[corr4(idx, 2), p4(idx,2)] = corr(vels(nindices1,1), rates(nindices1,idx));
		[corr4(idx, 3), p4(idx,3)] = corr(vels(pindices2,2), rates(pindices2,idx));
		[corr4(idx, 4), p4(idx,4)] = corr(vels(nindices2,2), rates(nindices2,idx));

	
		%If filename provided, plot density estimates of distributions
		if (length(fn_out) > 0)
			plot(vels(pindices1,1), rates(pindices1,idx), '.');
			title(['correlation = ' num2str(corr4(idx,1))]);
			saveplot(gcf, [fn_out '_channel_' num2str(idx) '_torque1+_corr_' num2str(corr4(idx,1)) '.eps']);
			plot(vels(nindices1,1), rates(nindices1,idx), '.');
			title(['correlation = ' num2str(corr4(idx,2))]);
			saveplot(gcf, [fn_out '_channel_' num2str(idx) '_torque1-_corr_' num2str(corr4(idx,2)) '.eps']);
			plot(vels(pindices2,2), rates(pindices2,idx), '.');
			title(['correlation = ' num2str(corr4(idx,3))]);
			saveplot(gcf, [fn_out '_channel_' num2str(idx) '_torque2+_corr_' num2str(corr4(idx,3)) '.eps']);
			plot(vels(nindices2,2), rates(nindices2,idx), '.');
