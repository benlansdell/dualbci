...
N = 100;
P = 20;			%Number of resamples to do to bootstrap for each trial
proprec = 0.5;
M = size(recs_info, 1);

null_success_rates = zeros(N, P);
actual_success_rates = zeros(N,1);
...
for idx = 1:N
	...
	cursor_all = data.data.stateHist.cursor(start_bin:end_bin,:);
	n_bins = size(cursor_all,1);

	%Randomly do a resampling
	for p = 1:P
		%Randomly pick a start point...
		start_point = round(rand()*(1-proprec)*n_bins);
		end_point = min(start_point+round(n_bins*proprec), n_bins);
		cursor = cursor_all(start_point:end_point,:);
...
%			end
		...
		null_success_rates(idx,p) = n_success/(rec_time*proprec);
	end
	...
	actual_success_rates(idx) = n_act_success/rec_time;
end

max_null_success_rates = max(null_success_rates,[],2);
sum(actual_success_rates>max_null_success_rates)

alph = 5;
sum(actual_success_rates>alph*mean(null_success_rates, 2))