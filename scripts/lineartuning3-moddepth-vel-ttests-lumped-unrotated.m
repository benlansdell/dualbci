...
' 1, 0), ep1.value, ep2.value, ep5.value FROM '...
...
consts = cell2mat(bci_data.Data(:,8:10));
moddepth = all_r2./consts;
...
mc1bc_diffsize = moddepth(unrot,1) - moddepth(unrot,2);
mcdc_diffsize = moddepth(unrot,1)- moddepth(unrot,3);
bcdc_diffsize = moddepth(unrot,2)- moddepth(unrot,3);

bci_mc1bc_diffsize = moddepth(bci,1)- moddepth(bci,2);
bci_mcdc_diffsize = moddepth(bci,1)- moddepth(bci,3);
bci_bcdc_diffsize = moddepth(bci,2)- moddepth(bci,3);

nonbci_mc1bc_diffsize = moddepth(nonbci,1)- moddepth(nonbci,2);
nonbci_mcdc_diffsize = moddepth(nonbci,1)- moddepth(nonbci,3);
nonbci_bcdc_diffsize = moddepth(nonbci,2)- moddepth(nonbci,3);
...
saveplot(gcf, './worksheets/2016_06_10-resultsforpaper/tuningmoddepthBCI-nonBCI-boxplot-unrotated.eps')

%correlations
corrsbci(1) = corr(moddepth(bci,1), moddepth(bci,2));
corrsbci(2) = corr(moddepth(bci,1), moddepth(bci,3));
corrsbci(3) = corr(moddepth(bci,2), moddepth(bci,3));

corrsnonbci(1) = corr(moddepth(nonbci,1),moddepth(nonbci,2));
corrsnonbci(2) = corr(moddepth(nonbci,1),moddepth(nonbci,3));
corrsnonbci(3) = corr(moddepth(nonbci,2),moddepth(nonbci,3));
...
colormap(colors)
subplot(2,2,1)
scatter(moddepth(unrot,1), moddepth(unrot,2), [], cc(unrot))
xlabel('strength MC1')
ylabel('strength BC1')
%title(['corr bci: ' num2str(corrsbci(1)) ' corr nonbci: ' num2str(corrsnonbci(1))])
subplot(2,2,2)
scatter(moddepth(unrot,1), moddepth(unrot,3), [], cc(unrot))
xlabel('strength MC1')
ylabel('strength DC')
%title(['corr bci: ' num2str(corrsbci(2)) ' corr nonbci: ' num2str(corrsnonbci(2))])
subplot(2,2,3)
scatter(moddepth(unrot,2), moddepth(unrot,3), [], cc(unrot))
xlabel('strength BC1')
ylabel('strength DC')
%title(['corr bci: ' num2str(corrsbci(3)) ' corr nonbci: ' num2str(corrsnonbci(3))])
saveplot(gcf, './worksheets/2016_06_10-resultsforpaper/tuningmoddepthBCI-nonBCI-scatter-unrotated.eps')