...
%a = exec(conn, ['SELECT fl1.size, fl2.size, fl5.size, et1.`tuning_type`, rec1.`successrate`, rec2.`successrate`, '...
...
%
%save('./scripts/fig2c.mat')
load('./scripts/fig2c.mat')
...
mc1bc_diffsize = abs(all_r2(rot,1) - all_r2(rot,2));
mcdc_diffsize = abs(all_r2(rot,1)- all_r2(rot,3));
bcdc_diffsize = abs(all_r2(rot,2)- all_r2(rot,3));
mc1mc2_diffsize = abs(all_r2(rot,1)- mc2size(rot,1));
...
%Make violin plot instead....
figure 
subplot(2,2,1)
violinplot(bci_mc1bc_diffsize, 'c1', 'ShowData', false);
ylim([0, 1])
title('BCI. MC-BC')
subplot(2,2,2)
violinplot(nonbci_mc1bc_diffsize, 'c1', 'ShowData', false);
ylim([0, 1])
title('Non BCI. MC-BC')
subplot(2,2,3)
violinplot(bci_mcdc_diffsize, 'c1', 'ShowData', false);
ylim([0, 1])
title('BCI. MC-DC')
subplot(2,2,4)
violinplot(nonbci_mcdc_diffsize, 'c1', 'ShowData', false);
title('Non BCI. MC-DC')
ylim([0, 1])
saveplot(gcf, './figures/tuningstrengthBCI-nonBCI-violin-rotated.eps')

%Combine BCI and non-BCI data:
figure 
subplot(1,3,1)
violinplot(mc1mc2_diffsize, 'c1', 'ShowData', false);
ylim([0, 1])
title('MC-MC2')
subplot(1,3,2)
violinplot([bci_mc1bc_diffsize; nonbci_mc1bc_diffsize], 'c1', 'ShowData', false);
ylim([0, 1])
title('MC-BC')
subplot(1,3,3)
violinplot([bci_mcdc_diffsize; nonbci_mcdc_diffsize], 'c1', 'ShowData', false);
ylim([0, 1])
title('MC-DC')
saveplot(gcf, './figures/tuningstrength-violin-rotated.eps')
