function processLinearControl(conn, modelID, blackrock, nevfile, paramcode, threshold, reftorqdist, units, rerun)
  		 %processLinearControl(conn, modelID, blackrock, nevfile, paramcode, threshold, reftorqdist, allunits, rerun);

	%Control for different torque output between conditions by sampling points 
	%according to reference torque distribution and fitting a linear model
	...
	%make a resampled processed structure based on reference distribution
	processed = resampleProcessed(processed, ceil((dur+testdur)/binsize), reftorqdist);
	...
	d = ['processLinearControl: Fitting model ' num2str(modelID) ' nevfile ' nevfile];
	display(d);
	writelog(logfile, d);
	...
	for idx = 1:nU
		...
		if ~strcmp(previous.Data{1}, 'No Data')
			if ~rerun 
				d = ['processLinear: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Skipping'];
				display(d);
				writelog(logfile, d);
				continue
			elseif length(previous.Data) > 1
				d = ['processLinear: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Trying to overwrite '...
					' old fit, but found too many matching fits for this model to unambiguously replace with new data. Skipping'];
				display(d);
				writelog(logfile, d);
				continue				
			else
				d = ['processLinear: rerun = 1: Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Deleting and '...
				're-entering.'];
				display(d);
				writelog(logfile, d);
