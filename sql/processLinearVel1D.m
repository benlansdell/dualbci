function processLinearVel1D(conn, modelID, blackrock, nevfile, paramcode, threshold, units, rerun)
	logfile = './sqllog.txt';
	...
	for idx = 1:nU
		...
		if ~strcmp(previous.Data{1}, 'No Data')
			if ~rerun 
				d = ['processLinearVel1D: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Skipping'];
				display(d);
				writelog(logfile, d);
				continue
			elseif length(previous.Data) > 1
				d = ['processLinearVel1D: WARNING Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Trying to overwrite '...
					' old fit, but found too many matching fits for this model to unambiguously replace with new data. Skipping'];
				display(d);
				writelog(logfile, d);
				continue				
			else
				d = ['processLinearVel1D: rerun = 1: Model ' num2str(modelID) ' nevfile ' nevfile ' and unit ' unit ' already analysed. Deleting and '...
				're-entering.'];
				display(d);
				writelog(logfile, d);
