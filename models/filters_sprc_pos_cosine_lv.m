function data = filters_sprc_pos_cosine_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	...
	%
	%Transfrom stimulus to Fourier space (ie. the GLM will be fit to cosine basis functions)
	%
	%Usage:
	%	data = filters_sprc_pos_cosine_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sprc_pos_cosine_lv(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_pos*steps_pos)
			...
			torqueRU = fft(torqueRU);
			torqueFE = fft(torqueFE);
			shist = shist;
