function data = filters_sp_vel_1D(processed, nK_sp, nK_vel, axe, dt_sp, dt_vel)
	...
	%
	%Usage:
	%	data = filters_sp_vel(processed, nK_sp, nK_vel, axe, dt_sp, dt_vel)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_vel = 0.05;
	%	data = filters_sp_vel(pre.processed, nK_sp, nK_vel, axe, dt_sp, dt_vel);

	if (nargin < 5) dt_sp = processed.binsize; end
	if (nargin < 6) dt_vel = processed.binsize; end
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_vel*steps_vel)
			...
			%Form stim vector
			if axe == 1
				data.X(idx,j,:) = [shist' torqueRU'];
			else
				data.X(idx,j,:) = [shist' torqueFE'];
			end
		end
	end
