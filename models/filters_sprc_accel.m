function data = filters_sprc_accel(processed, nK_sp, nK_acc, dt_sp, dt_acc)
	...
	%
	%Usage:
	%	data = filters_sprc_accel(processed, nK_sp, nK_acc, dt_sp, dt_acc)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_acc = 0.05;
	%	data = filters_sprc_accel(pre.processed, nK_sp, nK_acc, dt_sp, dt_acc);
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_acc*steps_acc)
			...
			torqueRU = processed.ddtorque(j:steps_acc:(j+nK_acc*steps_acc-1),1);
			torqueFE = processed.ddtorque(j:steps_acc:(j+nK_acc*steps_acc-1),2);
