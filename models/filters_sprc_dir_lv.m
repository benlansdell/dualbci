function data = filters_sprc_dir_lv(processed, nK_sp, nK_dir, dt_sp, dt_dir)
	...
	%
	%Usage:
	%	data = filters_sprc_dir_lv(processed, nK_sp, nK_dir, dt_sp, dt_dir)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_dir = 0.05;
	%	data = filters_sprc_dir_lv(pre.processed, nK_sp, nK_dir, dt_sp, dt_dir);
	...
	data.k{1,3} = dt_sp;
	data.k{2,1} = 'RU pos'; 
	data.k{2,2} = (1:nK_dir) + nK_rc;
	data.k{2,3} = dt_dir;
	data.k{3,1} = 'FE pos'; 
	data.k{3,2} = (1:nK_dir) + nK_rc + nK_dir;
	data.k{3,3} = dt_dir;
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_dir*steps_dir)
			...
			torqueRU = processed.cursor(j:steps_dir:(j+(nK_dir-1)*steps_dir),1);
			torqueFE = processed.cursor(j:steps_dir:(j+(nK_dir-1)*steps_dir),2);
