...
%% Auto-caching of NEV objects - by C. Matlack 2013-12-13 %%%%%%%%%%%%%%%%%
% Note that calling openNEV() without arguments will run this code,
% clearing anything really old from the cache.
global NEVCACHE
try
if isempty(NEVCACHE)
    NEVCACHE.files = [];        % full file paths as passed to openNEV
    NEVCACHE.lastRead = [];     % 6-element time vectors returned by clock()
    NEVCACHE.nevs = [];         % NEV objects
else if ~isempty(NEVCACHE.nevs)
    % 10 minute lifespan
    keep = find(cellfun(@(t) etime(clock,t) < 600, NEVCACHE.lastRead));
    % 10 object limit
    keep = keep(max(1,length(keep)-15):end);
    % make logical index
    keep = logical(ismember(1:length(NEVCACHE.files),keep));
    
    if sum(keep) ~= length(NEVCACHE.nevs)
        cellfun(@(f) fprintf('Dropping %s\n',f{1}),regexp(NEVCACHE.files(~keep), '[^/]+[\d]{3}','match'))
        NEVCACHE.files = NEVCACHE.files(keep);
            NEVCACHE.nevs = NEVCACHE.nevs(keep);
            NEVCACHE.lastRead = NEVCACHE.lastRead(keep);
    end
        
%         [s,hname] = system('hostname');
%         % min 500mb free in linux
%         if strcmp('charlie-461',hname(1:end-1))
%             fprintf('%d mb free...\n',round(freemem));
%             if freemem() < 500
%                 while (freemem() < 500)
%                     NEVCACHE.files = NEVCACHE.files(2:end);
%                     NEVCACHE.nevs = NEVCACHE.nevs(2:end);
%                     NEVCACHE.lastRead = NEVCACHE.lastRead(2:end);
%                     fprintf('Dumped NEV from cache; now %dmb free...\n',round(freemem))
%                 end
%             end
%         end
    end
...
if nargin == 0 
    return          % allows a call to check for cache dump conditions
end
for i=1:length(varargin)
    switch lower(varargin{i})
        case 'flush'                    % flush NEV cache
            NEVCACHE.files = [];
            NEVCACHE.lastRead = [];
            NEVCACHE.nevs = [];
            fprintf('NEV cache flushed.\n');
        case 'report'
...
if exist(matPath, 'file') == 2 && strcmpi(Flags.NoMAT, 'yesmat')
    ...
    %% Check the cache before loading .mat file
    [tf, idx] = ismember(fileFullPath(1:end-4), NEVCACHE.files);
    if tf
        if strcmpi(Flags.Report, 'report')
            fprintf('Loading from NEV cache\n');
        end
        NEV = NEVCACHE.nevs{idx};
        NEVCACHE.lastRead{idx} = clock();
    else
        if strcmpi(Flags.Report, 'report')
            fprintf('(Cache miss) Loading MAT for NEV...');
        end
        load(matPath);
        [NEV.MetaTags.FilePath, NEV.MetaTags.Filename] = fileparts(fileFullPath);                       %%% By C. Matlack 2013-11-28
    end
    ...
    if isempty(NEV.Data.Spikes.Waveform) && strcmpi(Flags.ReadData, 'read')
        ...
        if ~tf      % if not cached, add to cache
            newIdx = length(NEVCACHE.nevs)+1;
            NEVCACHE.nevs{newIdx} = NEV;
            NEVCACHE.files{newIdx} = [NEV.MetaTags.FilePath filesep NEV.MetaTags.Filename];
            NEVCACHE.lastRead{newIdx} = clock();
        end
        if ~nargout
...
if strcmpi(Flags.SaveFile, 'save')
    if exist(matPath, 'file') == 2 && strcmpi(Flags.Overwrite, 'nooverwrite')
        ...
        if strcmpi(overWrite, 'y')
            fprintf('Saving MAT file. This may take a few seconds...');
